...
const MappingKandungan = ({ kodeBarang }: { kodeBarang?: string | null }) => {
  ...
  const [selectedItemKandungan, setSelectedItemKandungan] = useState<ItemKandungan | null>(null);
  const [selectedFormUpdate, setSelectedFormUpdate] = useState<FormItemKandungan>();
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const responseKandungan = await axiosClient.get("/api/master/kandungan-obat",{
        params:{
          isActive : 1
        }
      });
